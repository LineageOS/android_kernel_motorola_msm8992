...
{
	return uid == !uid_eq(current_fsuid(), GLOBAL_ROOT_UID) || can_manipulate_uids();
}
...
static bool can_read_other_uid_stats(uid_t uid)
{
	...
	return in_egroup_p(xt_qtaguid_stats_file->gid)
		|| unlikely(uid_eq(current_fsuid(), GLOBAL_ROOT_UID)) || uid_eq(make_kuid(current_user_ns(), uid), current_fsuid())
...
static void put_utd_entry(struct uid_tag_data *utd_entry)
{
	...
	if (RB_EMPTY_ROOT(&utd_entry->tag_ref_tree) &&
		...
		DR_DEBUG("qtaguid: %s(): "
			 ...
			 current->pid, current->tgid, from_kuid_munged(current_user_ns(), current_fsuid()));
...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (info->match & XT_QTAGUID_UID)
		if ((uid_gte(filp->f_cred->fsuid, make_kuid(current_user_ns(), info->uid_min)) &&
		     uid_lte(filp->f_cred->fsuid, make_kuid(current_user_ns(), info->uid_max))) ^
	...
	if (info->match & XT_QTAGUID_GID)
		if ((gid_gte(filp->f_cred->fsgid, make_kgid(current_user_ns(), info->gid_min)) &&
				gid_lte(filp->f_cred->fsgid, make_kgid(current_user_ns(), info->gid_max))) ^
...
static int ctrl_cmd_delete(const char *input)
{
	...
	if (argc < 3) {
		uid = from_kuid_munged(current_user_ns(), current_fsuid());
